...
import transformers
from transformers import AutoTokenizer, AutoModelForCausalLM, PreTrainedModel, AutoModelForSequenceClassification
...
class Samples:
    ...
    prompt: any
    answer: any
    # Optional 表示一个变量既可以是指定的类型，也可以是None
    attention_mask: Optional[torch.LongTensor]
    action_mask: Optional[torch.BoolTensor]
    # Union 表示一个变量可以是指定的类型之一
...
class GRPOArguments:
    ...
    save_steps = 100
    epoch = 3
    num_generations = 8 # 组内的样本数
    max_prompt_length = 256 # 最大输入长度
    max_generate_length = 256 # 最大输出长度
    # List[float] 也是一种类型提示（Type Hint），它的意思是： 这是一个列表（List），且列表里的每一个元素都必须是浮点数（float）
    reward_weights: List[float] = None # 奖励函数的权重(多个奖励函数)
    beta = 0.0 # KL散度的系数，为0则忽略KL散度，即不适用参考模型
    clip_eps = 0.2 # 截断范围
...
class GRPOTrainer:
    def __init__(self,
                 model = None,
                 # 这个变量，要么你给我一组 函数的名字（字符串） ，要么你直接给我一组 函数对象 ，二者选其一
        ...
        if reward_tokenizers is None:
            ...
            if len(reward_tokenizers) != len(reward_funcs):
                raise ValueError("reward_tokenizer 必须和 reward_funcs 长度相同")
            
        """
        逐行拆解： 
        1. for i, (reward_tokenizer, reward_func) in enumerate(zip(reward_tokenizer, reward_funcs)):
        - 这里是在遍历所有的奖励函数和它们对应的分词器。
        - reward_funcs 列表里可能既有简单的 Python 函数，也有复杂的 PreTrainedModel （预训练模型）

        2. if isinstance(reward_func, PreTrainedModel):
        - 检查类型 ：判断当前的奖励函数是不是一个预训练模型（比如一个专门打分的 BERT 或 GPT 模型）。
        - 如果它只是一个普通函数（比如检查代码是否能运行的脚本），则跳过这一步。

        3. if reward_tokenizer is None:
        - 自动加载 ：如果在初始化 GRPOTrainer 时没有手动传入分词器，代码会尝试“自力更生”。
        - AutoTokenizer.from_pretrained(reward_func.config.name_or_path) ：它会读取模型的配置，找到这个模型当初训练时使用的路径或名字，然后自动从网上（或本地缓存）下载并加载匹配的分词器。这样能确保模型和分词器完全对应，不会出现 ID 错乱。 
        
        4. if reward_tokenizer.pad_token_id is None:
        - 处理填充符 ：很多生成式模型（如 GPT）默认没有 pad_token （填充符）。
        - reward_tokenizer.pad_token = reward_tokenizer.eos_token ：如果缺少填充符，就用 eos_token （结束符）来充当填充符。这是 NLP 中的常见做法，目的是为了让不同长度的句子能拼成一个批次（Batch）进行并行计算。
        """
            
        for i, (reward_tokenizer, reward_func) in enumerate(zip(reward_tokenizers, reward_funcs)):
            if isinstance(reward_func, PreTrainedModel):
                if reward_tokenizers is None:
                    reward_tokenizer = AutoTokenizer.from_pretrained(reward_func.config.name_or_path)
    ...
    def generate_samples(self, inputs):
        ...
        for prompt, answer in zip(prompts, answers):
            ...
            input_text = self.tokenizer.apply_chat_template(
                [{"role": "system", "content": SYSTEM_PROMPT}, {"role": "user", "content": prompt}], tokenize=False, add_generation_prompt=True
            )

            # 生成一个group的输入数据
            inputs = self.tokenizer([input_text] * self.args.num_generations, padding='max_length', max_length=self.args.max_length, truncation=True, return_tensors='pt')
            ...
            with torch.no_grad():
                ...
                if prompt_response_ids.size(1) >= max_length:
                    ...
                    prompt_response_ids = torch.cat([prompt_response_ids, torch.full((prompt_response_ids.size(0), max_length - prompt_response_ids.size(1)), self.tokenizer.pad_token_id, dtype=torch.long)], dim=1, fill_value=self.tokenizer.pad_token_id)
    ...
    def generate_experiences(self, inputs):
        ...
        self.model.eval()
        sample_list = self.generate_samples(inputs)
            ...
            with torch.no_grad():
                ...
                for i, (reward_func, reward_tokenizer) in enumerate(zip(self.reward_funcs, self.reward_tokenizers)):
                    if isinstance(reward_func, PreTrainedModel):
                        ...
                        answer = [answer] * len(prompt_texts)
...
"""
    ...
    prompt_dataset = GSM8kDataset('', tokenizer)
