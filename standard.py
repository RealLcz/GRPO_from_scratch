...
class GRPOArguments:
    ...
    dataset_name = 'swulling/gsm8k_chinese'
    wandb_project = 'grpo'
    wandb_entity = None
    wandb_run_name = 'grpo-qwen2.5-0.5b'
    wandb_mode = 'online'
    wandb_watch = True
...
class GRPOTrainer:
    def __init__(self,
        ...
        for reward_func in reward_funcs:
            if isinstance(reward_func, PreTrainedModel):
                name = getattr(reward_func.config, "name_or_path", None) or getattr(reward_func.config, "_name_or_path", None) or reward_func.__class__.__name__
            else:
                name = getattr(reward_func, "__name__", reward_func.__class__.__name__)
        ...
        # 模型更新的次数
        self.update_steps = 0 
        if wandb.run and self.args.wandb_watch:
            wandb.watch(self.model, log="all", log_freq=self.args.wandb_watch_freq)
    ...
    def generate_experiences(self, inputs):
        ...
        action_mask_ratio_list = []
        old_action_log_probs_list = []
        ref_action_log_probs_list = []
        rewards_per_func_mean_list = []
        ...
        for samples in samples_list:
            ...
            with torch.no_grad():
                ...
                response_length_list.append(response_length.detach())
                action_mask_ratio_list.append(action_mask.float().mean().detach())
                old_action_log_probs_list.append(old_action_log_probs.detach())
                if self.ref_model:
                    ref_action_log_probs_list.append(ref_action_log_probs.detach())
                rewards_per_func_mean_list.append(rewards_per_func.mean(dim=1).detach())
        rewards_all = torch.cat(rewards_list)
        advantages_all = torch.cat(advantages_list)
        response_lengths_all = torch.cat(response_length_list)
        action_mask_ratio = torch.stack(action_mask_ratio_list).mean()
        old_action_log_probs_all = torch.cat(old_action_log_probs_list)
        metrics = {
            "rewards_mean": rewards_all.mean(),
            "rewards_std": rewards_all.std(),
            "rewards_min": rewards_all.min(),
            "rewards_max": rewards_all.max(),
            "advantages_mean": advantages_all.mean(),
            "advantages_std": advantages_all.std(),
            "response_length_mean": response_lengths_all.float().mean(),
            "response_length_max": response_lengths_all.max(),
            "action_mask_ratio": action_mask_ratio,
            "old_action_log_probs_mean": old_action_log_probs_all.mean(),
        }
        if rewards_per_func_mean_list:
            rewards_per_func_mean = torch.stack(rewards_per_func_mean_list).mean(dim=0)
            for i, name in enumerate(self.reward_func_names):
                metrics[f"reward_func/{name}_mean"] = rewards_per_func_mean[i]
        if self.ref_model:
            ref_action_log_probs_all = torch.cat(ref_action_log_probs_list)
            metrics["ref_action_log_probs_mean"] = ref_action_log_probs_all.mean()
    ...
    def train_step(self, model, inputs, optimizer, step):
        ...
        if (step + 1) % self.args.gradient_accumulation_steps == 0:
            ...
            if wandb.run and self.update_steps % self.args.log_every_steps == 0:
                ...
                if torch.cuda.is_available():
                    log_data["cuda_memory_allocated_mb"] = torch.cuda.memory_allocated() / 1024 / 1024
                    log_data["cuda_memory_reserved_mb"] = torch.cuda.memory_reserved() / 1024 / 1024
                wandb.log(log_data, step=self.update_steps)
        ...
        for _ in range(self.args.epoch):
            ...
            for idx, batch in enumerate(dataloader):
                if stop_training:
                    break
                if "prompt" in batch:
                    seen_samples += len(batch["prompt"])
                else:
                    seen_samples += self.args.batch_size
                ...
                del inputs
                if seen_samples >= self.args.max_train_samples:
                    stop_training = True
                    break
    def save_model(self):
...
if __name__ == "__main__":
    ...
    args.dataset_name = 'swulling/gsm8k_chinese'
    args.wandb_project = 'grpo'
    args.wandb_run_name = 'grpo-qwen2.5-1.5b'
    args.wandb_entity = None
    args.wandb_mode = 'online'
    args.wandb_watch = True
    args.wandb_watch_freq = 200
...
"""
    ...
    wandb.init(project=args.wandb_project, entity=args.wandb_entity, name=args.wandb_run_name, mode=args.wandb_mode, config=config)
    # 策略模型
    ...
    prompts_dataset = GSM8KDataset(ds, tokenizer)
